					...
					{userData.map((item, index) => (
						<Box key={index} className="user-activity">
							<Box display="flex" p={{ xs: 2, md: 3 }}>
								<Box>
									<Avatar className={classes.large} alt="img" src={require(`assets/Images/avatars/${item.userimage}`).default} />
								</Box>
									...
									<Fragment>
										{item.postimage && item.postimage ?
											<Box mt={2} className={classes.thumbWrap}>
												<img src={require(`assets/Images/${item.postimage}`).default} height={300} className="img-fluid" alt="user-images" />
											</Box>
											:
											null
										}
